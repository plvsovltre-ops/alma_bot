...
# --- 2. ВЫБОР ЯЗЫКА (НОВЫЙ ФУНКЦИОНАЛ) ---
# Создаем контейнер для выбора языка
with st.container():
    ...
    for file_path in files:
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                # Добавляем маркер начала документа, но скрываем имя файла в инструкции ниже
...
# Очищаем историю, если пользователь сменил язык (опционально, но удобно)
...
# Приветственное сообщение при пустом чате
if not st.session_state.messages:
    if "Русский" in selected_lang:
        welcome = "Здравствуйте! Опишите проблему (например: *'Стройка на склоне'*, *'Рубят деревья'*)."
    else:
        welcome = "Сәлеметсіз бе! Мәселені сипаттаңыз (мысалы: *'Тау бөктеріндегі құрылыс'*, *'Ағаштарды кесу'*)."
...
# Обработка вопроса
prompt_text = "Введите ваш вопрос..." if "Русский" in selected_lang else "Сұрағыңызды енгізіңіз..."
if prompt := st.chat_input(prompt_text):
    ...
    with st.chat_message("assistant"):
        ...
        ТВОЯ ЗАДАЧА: Консультировать по вопросам защиты природы Алматы, используя Базу Знаний.
        
        ТВОЯ БАЗА ЗНАНИЙ:
        {knowledge_base}
        
        СТРОГАЯ ИНСТРУКЦИЯ ПО ФОРМАТУ:
        1. ЯЗЫК ОТВЕТА: {target_lang}. Отвечай ТОЛЬКО на этом языке. Не используй второй язык.
        2. ИСТОЧНИКИ: Ссылайся ТОЛЬКО на официальные названия законов и кодексов (например: "Экологический кодекс РК", "Парижское соглашение").
        3. ЗАПРЕТ: НИКОГДА не пиши названия файлов (например, "05_crime_code.txt", "source 1"). Это техническая информация, пользователю она не нужна. Вместо "Файл 10" пиши "Акты и поручения Президента".
        4. ЕСЛИ НАРУШЕНИЕ СЕРЬЕЗНОЕ: Предложи пошаговый алгоритм действий (Сценарий А или Б).
        5. Не выдумывай законы, которых нет в тексте.
        """
        ...
        try:
            response = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    temperature=0.0,
                    max_output_tokens=8000, # Увеличенный лимит для полных ответов
                    safety_settings=[
