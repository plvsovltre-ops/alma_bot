...
# Словарь для замены технических имен файлов на человеческие названия
FILE_MAPPING = {
    ...
    for file_path in files:
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                # Если файл есть в словаре, берем красивое имя. Если нет — просто имя без .txt
                doc_title = FILE_MAPPING.get(filename_raw, filename_raw.replace(".txt", "").replace("_", " ").upper())
...
if prompt := st.chat_input(prompt_text):
    ...
    with st.chat_message("assistant"):
        ...
        2. ИСТОЧНИКИ: При цитировании используй ТОЛЬКО названия документов, указанные после "--- ДОКУМЕНТ:".
        3. ЗАПРЕТ: НИКОГДА не упоминай имена файлов (вроде "01_land_code.txt").
        4. ЕСЛИ ЕСТЬ ФОТО: Сначала проанализируй его на предмет нарушений (склоны, вырубка, техника).
        5. ЕСЛИ НАРУШЕНИЕ СЕРЬЕЗНОЕ: Предложи пошаговый алгоритм (Сценарий А/Б).
        """
